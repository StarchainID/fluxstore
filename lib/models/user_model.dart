...
class UserModel with ChangeNotifier {
        ...
        case apple.AuthorizationStatus.authorized:
          {
            user = await _service.api
                .loginApple(token: String.fromCharCodes(result.credential!.identityToken!));
  ...
  Future<void> saveUser(User? user) async {
    try {
      if (Services().firebase.isEnabled && kFluxStoreMV.contains(serverConfig['type'])) {
  ...
  Future<void> createUser({
    ...
    try {
      loading = true;
      notifyListeners();
      Services().firebase.createUserWithEmailAndPassword(email: username!, password: password!);
  ...
  Future<void> login({username, password, required Function success, Function? fail}) async {
    try {
      loading = true;
      notifyListeners();
      ...
      Services().firebase.loginFirebaseEmail(email: username, password: password);
