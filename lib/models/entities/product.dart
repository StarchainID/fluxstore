...
class Product {
    ...
    try {
      ...
      if (type == 'variable') {
        ...
        onSale = salePrice != null &&
            ...
            regularPrice != null &&
            double.parse(regularPrice!) > double.parse(parsedJson['price'].toString());
      }
      ...
      if (parsedJson['attributesData'] != null) {
        try {
          parsedJson['attributesData'].forEach(
              (element) => vendorAdminProductAttributes.add(ProductAttribute.fromJson(element)));
        } catch (e) {
          printLog(e);
        }
      }
                ...
                infors
                        .firstWhereOrNull((ProductAttribute productAttribute) =>
                            ...
                            (productAttribute.name == attr.name ||
                                productAttribute.name!.toLowerCase() == attr.name!.toLowerCase() ||
      ...
      try {
        ...
        if (_categories != null && _categories is List && _categories.isNotEmpty) {
          ...
          try {
            if (item['key'] == '_product_addons') {
              ...
              final List<dynamic> values =
                  item['value'] is Map ? (item['value'] as Map).values.toList() : item['value'];
              addOns = [];
  ...
  Product.fromOpencartJson(Map<String, dynamic> parsedJson) {
    try {
      ...
      permalink = serverConfig['url']! + '/index.php?route=product/product&product_id=$id';
      regularPrice = parsedJson['price'];
      salePrice = parsedJson['special'];
      price = salePrice ?? regularPrice;
      onSale = salePrice != null;
      inStock = parsedJson['stock_status'] == 'In Stock' || int.parse(parsedJson['quantity']) > 0;
      averageRating =
          parsedJson['rating'] != null ? double.parse(parsedJson['rating'].toString()) : 0.0;
      ratingCount =
          parsedJson['reviews'] != null ? int.parse(parsedJson['reviews'].toString()) : 0.0 as int?;
      attributes = [];
  ...
  Product.fromPresta(Map<String, dynamic> parsedJson, apiLink) {
    try {
      ...
      totalSales = 0;
      regularPrice = (double.parse((parsedJson['price'] ?? 0.0).toString())).toStringAsFixed(2);
      salePrice =
          (double.parse((parsedJson['wholesale_price'] ?? 0.0).toString())).toStringAsFixed(2);
      price = (double.parse((parsedJson['wholesale_price'] ?? 0.0).toString())).toStringAsFixed(2);
      idShop =
          parsedJson['id_shop_default'] != null ? parsedJson['id_shop_default'].toString() : null;
      ...
      imageFeature = parsedJson['id_default_image'] != null
          ...
          : null;
      images = [];
      if (parsedJson['associations'] != null && parsedJson['associations']['images'] != null) {
      ...
      if (parsedJson['quantity'] != null && parsedJson['quantity'].toString().isNotEmpty) {
      ...
      if (parsedJson['attributes'] != null) {
        ...
        for (var i = 0; i < keys.length; i++) {
          attrs.add(
              ProductAttribute.fromPresta({'id': i, 'name': keys[i], 'options': res[keys[i]]}));
        }
        attributes = attrs;
      } else {
        attributes = [];
  ...
  Product.fromJsonStrapi(SerializerProduct model, apiLink) {
    try {
      ...
      imageFeature = images.isNotEmpty ? images[0] : apiLink(model.thumbnail!.url);
      ...
      if (model.productCategories != null) {
        categoryId =
            model.productCategories!.isNotEmpty ? model.productCategories![0].id.toString() : '0';
      } else {
        categoryId = '0';
      }
      onSale = model.isSale;
  ...
  Product.fromListingJson(Map<String, dynamic> json) {
    try {
      ...
      description = Tools.getValueByKey(json, DataMapping().ProductDataMapping['description']);
      ...
      averageRating = rate != null
          ? double.parse(double.parse(double.parse(rate.toString()).toString()).toStringAsFixed(1))
          : 0.0;

      regularPrice = Tools.getValueByKey(json, DataMapping().ProductDataMapping['regularPrice']);
      ...
      final reviews = Tools.getValueByKey(json, DataMapping().ProductDataMapping['totalReview']);
      ...
      if (DataMapping().ProductDataMapping['featured'] != null) {
        featured = Tools.getValueByKey(json, DataMapping().ProductDataMapping['featured']);
      }
      verified = false;
      if (DataMapping().ProductDataMapping['verified'] != null) {
        String? verifyText =
            Tools.getValueByKey(json, DataMapping().ProductDataMapping['verified']);
        if (verifyText == 'on' || verifyText == 'claimed') {
          verified = true;
        }
      }

      var list = Tools.getValueByKey(json, DataMapping().ProductDataMapping['gallery']);
