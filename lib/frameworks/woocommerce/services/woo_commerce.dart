...
class WooCommerceService extends BaseServices {
  ...
  Future<List<Category>> getCategoriesByPage({lang, page, limit}) async {
    try {
      var url =
          'products/categories?exclude=$kExcludedCategory&per_page=$limit&page=$page&hide_empty=true';
      ...
      if (kAdvanceConfig['isCaching'] &&
        ...
        if (verticalLayout != null &&
            ...
            ((verticalLayout['category'] != null &&
                ...
                (verticalLayout['tag'] != null && verticalLayout['tag'] == config['tag']))) {
  ...
  Future<List<Product>?> fetchProductsByCategory(
    ...
    try {
      ...
      var endPoint = 'products?status=publish&per_page=$ApiPageSize&page=$page&skip_cache=1';
      if (kAdvanceConfig['isMultiLanguages']) {
        endPoint += '&lang=$lang';
      }
  ...
  Future<User?> loginFacebook({String? token}) async {
    ...
    try {
      var endPoint = '$domain/wp-json/api/flutter_user/fb_connect/?second=$cookieLifeTime'
  ...
  @override
  Future<Null> createReview({String? productId, Map<String, dynamic>? data, String? token}) async {
    try {
      data!['product_id'] = productId;
      final response = await httpPost('$domain/wp-json/api/flutter_woo/products/reviews'.toUri()!,
  ...
  Future<List<ProductVariation>> getProductVariations(Product product,
    ...
    try {
      ...
      while (true) {
        var endPoint = 'products/${product.id}/variations?per_page=100&page=$page';
        if (kAdvanceConfig['isMultiLanguages']) {
          endPoint += '&lang=$lang';
        }
  ...
  Future<List<ShippingMethod>> getShippingMethods(
      {CartModel? cartModel, String? token, String? checkoutId, Store? store}) async {
    try {
      ...
      if (kVendorConfig['DisableVendorShipping'] == false) {
        ...
        line_items.forEach((element) {
          ...
          if ((store == null && product!.store == null) ||
              (store != null && product!.store != null && product.store!.id == store.id)) {
            params['line_items'] = [element];
          }
        });
      }
      var list = <ShippingMethod>[];
      final response = await httpPost('$domain/wp-json/api/flutter_woo/shipping_methods'.toUri()!,
          body: convert.jsonEncode(params), headers: {'Content-Type': 'application/json'});
      ...
      if (response is Map && isNotBlank(response['message'])) {
        ...
        for (var item in response) {
          var methodItem = {
            ...
            'label': item['title'],
            'cost': item['settings']['cost'] != null && item['settings']['cost']['value'] != null
  ...
  Future<List<PaymentMethod>> getPaymentMethods(
      {CartModel? cartModel, ShippingMethod? shippingMethod, String? token}) async {
    try {
      if (isBookingProduct(cartModel!)) {
      ...
      final response = await httpPost('$domain/wp-json/api/flutter_woo/payment_methods'.toUri()!,
          body: convert.jsonEncode(params), headers: {'Content-Type': 'application/json'});
      final body = convert.jsonDecode(response.body);
  ...
  Future<List<PaymentMethod>> getPaymentMethodsByWooApi() async {
    try {
      ...
      if (response is Map && isNotBlank(response['message'])) {
        ...
        for (var item in response) {
          if (item['enabled'] == true && item['id'] != 'wc-appointment-gateway') {
            var methodItem = {
              'id': item['id'],
  ...
  Future<PagingResponse<Order>> getMyOrders({User? user, dynamic cursor}) async {
    try {
      var response = await wcApi
          .getAsync('orders?customer=${user!.id}&per_page=20&page=$cursor&order=desc&orderby=id');
      var list = <Order>[];
      if (response is Map && isNotBlank(response['message'])) {
        throw Exception(response['message']);
      } else {
  ...
  @override
  Future<List<OrderNote>> getOrderNote({String? userId, String? orderId}) async {
    try {
      var response = await wcApi.getAsync('orders/$orderId/notes?customer=$userId&per_page=20');
      var list = <OrderNote>[];
      if (response is Map && isNotBlank(response['message'])) {
        throw Exception(response['message']);
      } else {
...
//          }
  ...
  Future<Order> createOrder(
      {CartModel? cartModel, UserModel? user, bool? paid, String? transactionId}) async {
    try {
      ...
      final response = await httpPost('$domain/wp-json/api/flutter_order/create'.toUri()!,
  ...
  Future updateOrder(orderId, {status, token}) async {
    try {
      final response = await httpPut('$domain/wp-json/api/flutter_order/update/$orderId'.toUri()!,
  ...
  Future<Order?> cancelOrder({Order? order, String? userCookie}) async {
    final newOrder =
        await Services().api.updateOrder(order!.id, status: 'cancelled', token: userCookie);
    return newOrder;
  }
  ...
  Future<User?> createUser({
    ...
    try {
      ...
      final response = await httpPost(
          '$domain/wp-json/api/flutter_user/sign_up/?insecure=cool&$isSecure'.toUri()!,
  ...
  Future<User?> login({username, password}) async {
    ...
    try {
      final response = await httpPost(
          ...
          body: convert.jsonEncode(
              {'seconds': cookieLifeTime.toString(), 'username': username, 'password': password}),
          headers: {'Content-Type': 'application/json'});
  ...
  Future<AfterShip> getAllTracking() async {
    final data = await httpGet('https://api.aftership.com/v4/trackings'.toUri()!,
  ...
  Future<Map<String, dynamic>?> getHomeCache(String? lang) async {
    try {
      ...
      if (config['HorizonLayout'] != null) {
        ...
        if (config['VerticalLayout'] != null && config['VerticalLayout']['data'] != null) {
  ...
  Future<User?> loginGoogle({String? token}) async {
    ...
    try {
      var endPoint = '$domain/wp-json/api/flutter_user/google_login/?second=$cookieLifeTime'
              '&access_token=$token$isSecure'
          .toUri()!;
  ...
  @override
  Future<String> getCheckoutUrl(Map<String, dynamic> params, String? lang) async {
    try {
      var str = convert.jsonEncode(params);
      var bytes = convert.utf8.encode(str);
      var base64Str = convert.base64.encode(bytes);

      final response = await httpPost('$domain/wp-json/api/flutter_user/checkout'.toUri()!,
          body: convert.jsonEncode({
            'order': base64Str,
          }),
          headers: {'Content-Type': 'application/json'});
      ...
      if (response.statusCode == 200 && body is String) {
        if (kPaymentConfig['EnableOnePageCheckout'] ||
            ...
            params['token'] == null) {
          Map<String, dynamic> checkoutPageSlug = kPaymentConfig['CheckoutPageSlug'];
  ...
  Future<String> submitForgotPassword({String? forgotPwLink, Map<String, dynamic>? data}) async {
    try {
      ...
      var response = await httpPost(endpoint,
          body: convert.jsonEncode(data), headers: {'Content-Type': 'application/json'});
  ...
  Future<Map<String, dynamic>?> getCurrencyRate() async {
    try {
      final response =
          await httpGet('$domain/wp-json/api/flutter_user/get_currency_rates'.toUri()!);
      var body = convert.jsonDecode(response.body);
  ...
  Future getCountries() async {
    try {
      final response = await httpGet('$domain/wp-json/api/flutter_user/get_countries'.toUri()!);
      var body = convert.jsonDecode(response.body);
      return body;
    } catch (err) {
      rethrow;
  ...
  Future getStatesByCountryId(countryId) async {
    try {
      final response = await httpGet(
          '$domain/wp-json/api/flutter_user/get_states?country_code=$countryId'.toUri()!);
      var body = convert.jsonDecode(response.body);
      return body;
    } catch (err) {
      rethrow;
  ...
  Future<List<dynamic>?>? getCartInfo(String? token) async {
    try {
      ...
      final response =
          await httpGet('$domain/wp-json/api/flutter_woo/cart?token=$base64Str'.toUri()!);
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
        return body;
      } else if (body['message'] != null) {
  ...
  Future syncCartToWebsite(CartModel cartModel, User? user) async {
    try {
      ...
      final response = await httpPost('$domain/wp-json/api/flutter_woo/cart'.toUri()!,
          body: convert.jsonEncode(params), headers: {'Content-Type': 'application/json'});
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
        return body;
  ...
  Future<Map<String, dynamic>> getCustomerInfo(String? id) async {
    try {
      ...
      if (response['billing'] != null) {
        ...
        billing.firstName = billing.firstName!.isEmpty ? address.firstName : billing.firstName;
        billing.lastName = billing.lastName!.isEmpty ? address.lastName : billing.lastName;
  ...
  Future<Map<String, dynamic>?> getTaxes(CartModel cartModel) async {
    try {
      ...
      final params =
          Order().toJson(cartModel, cartModel.user != null ? cartModel.user!.id : null, false);

      final response = await httpPost('$domain/wp-json/api/flutter_woo/taxes'.toUri()!,
          body: convert.jsonEncode(params), headers: {'Content-Type': 'application/json'});
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
        var taxes = <Tax>[];
  ...
  @override
  Future<bool> pushNotification(cookie, {receiverEmail, senderName, message}) async {
    try {
      var base64Str = EncodeUtils.encodeCookie(cookie);
  ...
  @override
  Future<PagingResponse<Order>>? getVendorOrders({required User user, dynamic cursor = 1}) async {
    try {
      ...
      final response = await httpGet(
          '$domain/wp-json/wc/v2/flutter/vendor-orders?page=$cursor&per_page=25&token=$base64Str'
              .toUri()!);
      printLog('$domain/wp-json/wc/v2/flutter/vendor-orders?page=$cursor&token=$base64Str'.toUri());
      var body = convert.jsonDecode(response.body);
      var list = <Order>[];
  ...
  @override
  Future<Product> createProduct(String? cookie, Map<String, dynamic> data) async {
    try {
      final response = await httpPost('$domain/wp-json/api/flutter_multi_vendor/product'.toUri()!,
          body: convert.jsonEncode(data),
  ...
  @override
  Future<List<Product>> getOwnProducts(String? cookie, {int? page, int? perPage}) async {
    try {
      final response = await httpPost(
  ...
  Future<dynamic> uploadImage(dynamic data, String? token) async {
    try {
      final response = await httpPost('$domain/wp-json/api/flutter_multi_vendor/media'.toUri()!,
          body: convert.jsonEncode(data),
          headers: {'User-Cookie': token!, 'Content-Type': 'application/json'});
      var body = convert.jsonDecode(response.body);
      if (body['message'] == null) {
  ...
  Future<Point?> getMyPoint(String? token) async {
    try {
      var base64Str = EncodeUtils.encodeCookie(token!);
      final response =
          await httpGet('$domain/wp-json/api/flutter_woo/points?token=$base64Str'.toUri()!);
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
  ...
  Future updatePoints(String? token, Order? order) async {
    try {
      final response = await httpPatch('$domain/wp-json/api/flutter_woo/points'.toUri()!,
  ...
  Future<List<Product>> getProductNearest(location) async {
    try {
      ...
      var urlReq = '$domain/wp-json/wp/v2/${DataMapping().kProductPath}?status=publish&_embed=true';
      if (lat != 0 || long != 0) {
        ...
        for (var item in convert.jsonDecode(response.body)) {
          ...
          for (var item in product.images) {
            if (!item.contains('http')) {
              var res = await httpGet('$domain/wp-json/wp/v2/media/$item'.toUri()!);
  ...
  Future<bool> createBooking(dynamic booking) async {
    if (booking.isAvaliableOrder && booking.isEmpty == false) {
      final response = await httpPost('$domain/wp-json/api/flutter_booking/checkout'.toUri()!,
  ...
  @override
  Future<List<String>> getSlotBooking(String? idProduct, String idStaff, String date) async {
    var urlAPI = '$domain/wp-json/api/flutter_booking/get_slots?product_id=$idProduct&date=$date';
  ...
  @override
  Future<Prediction> getPlaceDetail(Prediction prediction, String? sessionToken) async {
    try {
  ...
  @override
  Future<List<Prediction>> getAutoCompletePlaces(String term, String? sessionToken) async {
    try {
  ...
  Future<Product?> getProductByPermalink(String productPermalink) async {
    try {
      final response = await httpGet(
          '$domain/wp-json/api/flutter_woo/products/dynamic?url=$productPermalink'.toUri()!);
