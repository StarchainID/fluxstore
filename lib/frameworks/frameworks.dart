...
//Sync cart to website
  ...
  Widget renderSearchScreen() {
    return ChangeNotifierProvider<SearchModel>(
      ...
      builder: (context, _) {
        return SearchScreen(key: const Key('search'));
      },
    );
  }
  ...
  Widget renderHorizontalListItem(Map<String, dynamic> config) => ProductList(
        config: ProductConfig.fromJson(config),
        key: config['key'] != null ? Key(config['key']) : UniqueKey(),
      );
  ...
  Widget renderCommentLayout(int postId, kBlogLayout type) => const SizedBox();

  Widget renderCommentField(int postId) => const SizedBox();
