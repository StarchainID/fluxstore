...
import '../../../models/index.dart' show Blog, Product, Review, User;
...
class WCFMService extends WooCommerceService {
  ...
  Future<List<Review>> getReviewsStore({storeId}) async {
    try {
      var list = <Review>[];
      var response = await wcfmApi.getAsync('reviews?status_type=approved');
  ...
  Future<List<Store>> getNearbyStores(Prediction prediction) async {
    ...
    try {
      var response = await wcApi.getAsync(
          'flutter/wcfm-stores?page=1&per_page=100&wcfmmp_radius_lat=${prediction.lat}&wcfmmp_radius_lng=${prediction.long}');

      /// use this code if you would like to apply QueryRadiusDistance
      // var response = await wcApi.getAsync("flutter/wcfm-stores?page=1&per_page=100&wcfmmp_radius_lat=${prediction.lat}&wcfmmp_radius_lng=${prediction.long}&wcfmmp_radius_range=${kAdvanceConfig['QueryRadiusDistance']}");
