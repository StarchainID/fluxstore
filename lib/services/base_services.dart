...
abstract class BaseServices {
  ...
  Future<List<Product>?> fetchProductsLayout({required config, lang, userId}) async =>
      const <Product>[];
  ...
  Future<List<ProductVariation>?>? getProductVariations(Product product, {String? lang}) => null;
  ...
  Future<Null>? createReview({String? productId, Map<String, dynamic>? data, String? token}) =>
      null;
  ...
  Future<String>? getCheckoutUrl(Map<String, dynamic> params, String? lang) => null;
  ...
  Future? checkoutWithCreditCard(String? vaultId, CartModel cartModel, Address address,
  ...
  Future<PaymentSettings>? addCreditCard(
      PaymentSettingsModel paymentSettingsModel, CreditCardModel creditCardModel) {
    return null;
  }
  ...
  Future<Product>? createProduct(String? cookie, Map<String, dynamic> data) => null;
  ...
  Future<List<Prediction>>? getAutoCompletePlaces(String term, String? sessionToken) => null;

  Future<Prediction>? getPlaceDetail(Prediction prediction, String? sessionToken) => null;
  ...
  Future<List<String>>? getSlotBooking(String? idProduct, String idStaff, String date) => null;

  Future<PagingResponse<Blog>>? getBlogs(dynamic cursor) async {
    try {
      final param = '_embed&page=$cursor';
      final response = await httpGet('${blogApi.url}/wp-json/wp/v2/posts?$param'.toUri()!);
  ...
  Future<String?> createRazorpayOrder(params) async {
    try {
      var client = BasicAuthClient(RazorpayConfig['keyId'], RazorpayConfig['keySecret']);
      final response =
          await client.post('https://api.razorpay.com/v1/orders'.toUri()!, body: params);
  ...
  Future<void> updateOrderIdForRazorpay(paymentId, orderId) async {
    try {
      final token =
          base64.encode(latin1.encode('${RazorpayConfig['keyId']}:${RazorpayConfig['keySecret']}'));

      final body = {
        'notes': {'woocommerce_order_id': orderId}
      };
      await httpPatch('https://api.razorpay.com/v1/payments/$paymentId'.toUri()!,
          headers: {'Authorization': 'Basic ' + token.trim(), 'Content-Type': 'application/json'},
  ...
  Future<List<Blog>> fetchBlogsByCategory({categoryId, page, lang, order = 'desc'}) async =>
      <Blog>[];
